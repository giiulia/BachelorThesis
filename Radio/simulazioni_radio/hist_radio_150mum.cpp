...
int main (int argc, char ** argv){
    ...
    ifstream misure("sorgradio150mum.xy", ios::app);
    if(!misure.is_open()){
        cout<<"il file non si è aperto"<<endl;
    }
    while (misure >> energy >> counts)
    {
        h->Fill(energy, counts*f150);
        sum = sum + counts;
    }
    ...
    h->Draw ("hist");
    //c1.SetLogy();
    c1.Print ("spettro_radio_sim150mum.pdf", "pdf") ;
    ...
    c2.Print ("spettro_radio_sim150mum_zoom.pdf", "pdf") ;
